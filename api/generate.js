...
function buildSystemPrompt(params) {
  ...
  prompt += '3bis. Le header doit contenir un menu de navigation dont CHAQUE lien pointe vers une ancre (#id) correspondant a une vraie section presente plus bas dans la page (ex: si tu crees une section Galerie avec id="galerie", le menu doit avoir un lien Galerie vers #galerie). IMPORTANT: sur ordinateur (au-dessus de 768px), les liens du menu doivent etre affiches directement, horizontalement, dans le header, jamais caches derriere un bouton hamburger. Le bouton hamburger et le panneau qui se deroule ne doivent apparaitre QUE sur mobile (sous 768px), via une media query CSS qui masque le menu horizontal et affiche le hamburger en dessous de 768px. Le menu mobile (hamburger) doit contenir exactement les memes liens que le menu desktop. Jamais de lien vide, de # sans destination, ou de menu qui ne correspond pas aux sections reellement generees.\n';
  prompt += '3ter. N utilise JAMAIS d emojis (aucun symbole comme des visages, mains, coeurs, fusees etc) nulle part dans le site : ni dans les titres, ni dans les boutons, ni dans les listes, ni comme puces. Ca donne un rendu amateur/generique qui trahit un contenu genere par IA. Utilise a la place de vraies icones simples en SVG inline (traits fins, monochromes, sans emoji) si une puce visuelle est necessaire, ou simplement du texte sans decoration.\n';
